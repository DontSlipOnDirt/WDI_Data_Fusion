...
/**
 * {@link AttributeValueFuser} for the NumRating of {@link Book}s.
 *
 * @author Oliver Lehmberg (oli@dwslab.de)
 *
 */
public class NumRatingsFuserAverage extends AttributeValueFuser<Double, Book, Attribute> {

    public NumRatingsFuserAverage() {
        super(new Average<Book, Attribute>());
    }

    @Override
    public boolean hasValue(Book record, Correspondence<Attribute, Matchable> correspondence) {
        return record.hasValue(Book.NUM_RATINGS);
    }

    @Override
    public Double getValue(Book record, Correspondence<Attribute, Matchable> correspondence) {
        return record.getNumRatings();
    }
    ...
    public void fuse(RecordGroup<Book, Attribute> group, Book fusedRecord, Processable<Correspondence<Attribute, Matchable>> schemaCorrespondences, Attribute schemaElement) {
        ...
        if (fused.getValue() == null){
            fusedRecord.setNumRatings(0.0);
        }
        else {
            fusedRecord.setNumRatings(fused.getValue());
        }
        fusedRecord.setAttributeProvenance(Book.NUM_RATINGS,
